...
UP_KEY = 1048695#119#63232
DOWN_KEY = 1048691#115 #63233
LEFT_KEY = 1048673 #97 #63234
RIGHT_KEY = 1048676 #100 #63235
TURN_RIGHT = 1048677#101
TURN_LEFT = 1048689#113
...
if __name__=="__main__":
    ...
    parser.add_argument("--state_space",help="how many locations the paddle can have",default=10,type=int)
    ...
    gpu_flag = args.gpu
    paddle_size = int(image_size / state_space)
    ...
    with tf.Graph().as_default(), tf.device(device_string):
        ...
        with sess.as_default():
            ...
            def redraw_heatmap(x,y,angle):
                ...
                #load the screen
                test_screen =numpy.random.randn(image_size,image_size,3)/100
                #other paddle location
                other_paddle_location = numpy.random.randint(state_space,size=(2))
                other_paddle_location[0] *= paddle_size
                other_paddle_location[1] = int(64 *.1)
                test_screen[int(other_paddle_location[0]-paddle_size/2):int(other_paddle_location[0]+paddle_size/2),int(other_paddle_location[1]-1):int(other_paddle_location[1]+1),:] = 1
                #draw the balls
                yy, xx = numpy.mgrid[:64, :64]
                ...
                noop_action_values = numpy.zeros(args.state_space)
                down_action_values = numpy.zeros(args.state_space)
                up_action_values = numpy.zeros(args.state_space)
                total_action_values = numpy.zeros(args.state_space) #these are the overall value of each position
                for i in range(args.state_space):
                    #move the paddle to the top
                    #they are 13 pixels high, and start at pixel 5
                    paddle = [ 5+paddle_size*i,(5+paddle_size)+paddle_size*i,55,57]
                    test_screen[:,54:,:] = -.11
                ...
                total_action_values[-1] /= 2
                total_action_values[1:state_space-1] /= 3
                ...
                for i in range(args.state_space):
                    paddle = [ 5+paddle_size*ordered_total[i],(5+paddle_size)+paddle_size*ordered_total[i],55,57]
